...
const colors = [
  'bg-blue-200', 'bg-green-200', 'bg-red-200', 'bg-yellow-200', 'bg-indigo-200', 'bg-purple-200', 'bg-pink-200', 'bg-orange-200'
];
...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="animal in displayedAnimals" 
           :key="animal.id" 
           :class="[getRandomColor(), 'card']">
        <h3>{{ animal.name }}</h3>
        <p><strong>Favourite Fruit:</strong> {{ animal.favouriteFruit }}</p>
        <p><strong>Food required:</strong> {{ calculateFoodRequirement(animal) }} Kg</p>
      </div>
    </div>

    <button v-if="animalsToFeedToday.length > 8 && !showAllAnimals" 
            @click="showAllAnimals = true"
            class="mt-4 px-4 py-2 rounded bg-blue-500 text-white">
      Show More Animals
    </button>

    <button v-if="showAllAnimals"
